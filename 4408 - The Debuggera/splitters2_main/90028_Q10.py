...
def trim_trailing_empty(g: list[str]) -> list[str]:
    """Drop all-'.' trailing rows and columns; keeps (0,0) unchanged."""
    g = pad_grid(g)
    if not g:
        return ["."]
    n, m = len(g), len(g[0])
    grid = [list(r) for r in g]
    while m > 0 and all(grid[i][m - 1] == "." for i in range(n)):
        m -= 1
    while n > 0 and all(grid[n - 1][j] == "." for j in range(m)):
        n -= 1
    if n == 0 or m == 0:
        return ["."]
    return ["".join(grid[i][:m]) for i in range(n)]
...
def compose_binary_take(child: list[str]) -> list[str]:
    """Half collected off the top via '^' at (0, sc); half continues to the right into child."""
    child = pad_grid(child)
    hc, wc = len(child), len(child[0])
    sc = 1
    rows = max(hc + 2, 3)
...
def compose_ternary_two_full(child: list[str]) -> list[str]:
    """'^' collects 1/3; 'v' column then a '>' row below the child collects 1/3; child gets 1/3 on row 1."""
    child = pad_grid(child)
    hc, wc = len(child), len(child[0])
    sc = 1
    collect_row = hc + 2
...
def main():
    ...
    for _ in range(T):
        ...
        den = den_pow(A, B)
        assert 1 <= P <= den
        g = trim_trailing_empty(pad_grid(build_factory(P, A, B)))
        N, M = len(g), len(g[0])
