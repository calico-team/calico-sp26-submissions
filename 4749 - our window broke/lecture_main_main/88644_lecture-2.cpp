...
int main() {
    ...
    while (T--) {
        ...
        for (int r = 0; r < N; ++r) {
            for (int c = 0; c < M; ++c) {
                occ_prefix[r][c + 1] = occ_prefix[r][c] + (grid[r][c] == '#');
            }
        ...
        sort(pts.begin(), pts.end(),
             [](const Point& a, const Point& b) { return a.u < b.u; });
        ...
        auto can = [&](int D) -> bool {
            ...
            int left = 0, right = 0;
            int n = pts.size();
            int total_in_strip = 0;
            for (int u_low = min_u; u_low <= max_u - D; ++u_low) {
                int u_high = u_low + D;
                while (right < n && pts[right].u <= u_high) {
                    int idx = pts[right].v - min_v;
                    cnt_v[idx]++;
                    total_in_strip++;
                    right++;
                }
                while (left < n && pts[left].u < u_low) {
                    int idx = pts[left].v - min_v;
                    cnt_v[idx]--;
                    total_in_strip--;
                    left++;
                }
                if (total_in_strip < K) continue;
                if (D >= v_range) return true;
                int window_sum = 0;
        ...
        for (int u_low = min_u; u_low <= max_u - D_min; ++u_low) {
            ...
            for (int u_val = u_low; u_val <= u_high; ++u_val) {
                int idx = u_val - u_off;
                if (idx >= 0 && idx < u_len) {
                    strip.insert(strip.end(), points_by_u[idx].begin(), points_by_u[idx].end());
                }
            }
