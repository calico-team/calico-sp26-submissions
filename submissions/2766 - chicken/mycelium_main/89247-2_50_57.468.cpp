...
ll cp(ll p,ll q,ll x)
{
    if(p<=0||q<=0||x<0)return 0;
    ll z=min(x,min((p-1)/9,(q-1)/5));
    auto f=
    [&](ll i)
    {
        return (p-1-9*i)/5<=(q-1-5*i)/9;
...
void solve()
{
    ...
    while(t--)
    {
        ...
        if(h >0)
        {
            ...
            {
                ll u=cp(p2,q2,a);
                ll c0=min((p2-1)/5,(q2-1)/9)+1;
                ll r0=min(a,min((p2-1)/9,(q2-1)/5));
                u-=c0;
                u-=r0;
                u-= sm(2*d-1,9,5,1,min(a,k));
                an+=u;
            }
            else
            {
