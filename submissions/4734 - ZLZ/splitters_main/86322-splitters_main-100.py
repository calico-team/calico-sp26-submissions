    ...
    def give_to_another_conveyor(self, world_grid, width, height):
        ...
        for offset in offsets:
            ...
            if type(cell) is Conveyor:
                if offset == (0, 1):
                    direction_conveyor_should_have = "down"
                elif offset == (1, 0):
                    direction_conveyor_should_have = "right"
                elif offset == (-1, 0):
                    direction_conveyor_should_have = "left"
                elif offset == (0, -1):
                    direction_conveyor_should_have = "up"
                else:
                    raise Exception("Big Yahu Please Help me out")
                # print(cell.direction, offset)
                if cell.direction == direction_conveyor_should_have:
...
def solve():
    ...
    width = int(second_line[1])
    height = int(second_line[0])
    world = data[2:]
    print(world)
