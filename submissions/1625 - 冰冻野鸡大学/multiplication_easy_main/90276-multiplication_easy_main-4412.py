# WRITTEN BY CLAUDE OPUS 4.6 EXTENDED

#   /$$$$$$  /$$        /$$$$$$  /$$   /$$ /$$$$$$$  /$$$$$$$$
#  /$$__  $$| $$       /$$__  $$| $$  | $$| $$__  $$| $$_____/
# | $$  \__/| $$      | $$  \ $$| $$  | $$| $$  \ $$| $$
# | $$      | $$      | $$$$$$$$| $$  | $$| $$  | $$| $$$$$
# | $$      | $$      | $$__  $$| $$  | $$| $$  | $$| $$__/
# | $$    $$| $$      | $$  | $$| $$  | $$| $$  | $$| $$
# |  $$$$$$/| $$$$$$$$| $$  | $$|  $$$$$$/| $$$$$$$/| $$$$$$$$
#  \______/ |________/|__/  |__/ \______/ |_______/ |________/
                                                            
# (which should probably be allowed since contest ended and I have 0 clue how to do this)
# PLS NO BAN

# runtime error counter: 5
# timelimit error: 0
# wrong answer counter: 0

import sys
import math
import time
import heapq
from array import array

def main():
    t0 = time.time()
    data = sys.stdin.buffer.read().decode()
    tokens = data.split()
    pos = 0
    N = int(tokens[pos]); pos += 1
    M = int(tokens[pos]); pos += 1
    D = int(tokens[pos]); pos += 1

    sys.stderr.write(f"N={N} M={M} D={D}\n")
    sys.stderr.flush()

    if M == 0:
        sys.stdout.write(' '.join(['1']*N) + '\n0\n')
        return

    R = array('i', [0]*M)
    C = array('i', [0]*M)
    V = array('d', [0.0]*M)
    W = array('d', [0.0]*M)
    for i in range(M):
        R[i] = int(tokens[pos])-1; pos+=1
        C[i] = int(tokens[pos])-1; pos+=1
        V[i] = float(tokens[pos]); pos+=1
        W[i] = float(tokens[pos]); pos+=1
    del tokens
    del data

    # Build flat adjacency
    counts = array('i', [0]*N)
    for i in range(M):
        counts[R[i]] += 1
        counts[C[i]] += 1

    offsets = array('i', [0]*(N+1))
    for i in range(N):
        offsets[i+1] = offsets[i] + counts[i]

    total_adj = offsets[N]
    adj_ci = array('i', [0]*total_adj)
    adj_ov = array('i', [0]*total_adj)

    fill = array('i', [0]*N)
    for i in range(M):
        r = R[i]; c = C[i]
        p = offsets[r] + fill[r]
        adj_ci[p] = i; adj_ov[p] = c
        fill[r] += 1
        p = offsets[c] + fill[c]
        adj_ci[p] = i; adj_ov[p] = r
        fill[c] += 1
    del fill, counts

    LOG_MAX = math.log(1e9)
    _log = math.log
    _exp = math.exp
    _abs = abs

    def clamp_log(x):
        if x < 0.0: return 0.0
        if x > LOG_MAX: return LOG_MAX
        return x

    def log_to_int(x):
        if x < 0.0: x = 0.0
        elif x > LOG_MAX: x = LOG_MAX
        v = int(_exp(x) + 0.5)
        if v < 1: return 1
        if v > 1000000000: return 1000000000
        return v

    def total_pen_with_discard(a):
        pens_sum = 0.0
        if D <= 0:
            for i in range(M):
                pens_sum += W[i] * _abs(a[R[i]] * a[C[i]] - V[i]) / V[i]
            return pens_sum, []
        
        d = min(D, M)
        heap = []
        for i in range(M):
            p = W[i] * _abs(a[R[i]] * a[C[i]] - V[i]) / V[i]
            pens_sum += p
            if p > 0:
                if len(heap) < d:
                    heapq.heappush(heap, (p, i))
                elif p > heap[0][0]:
                    heapq.heapreplace(heap, (p, i))

        disc_sum = 0.0
        ds = []
        for p, idx in heap:
            ds.append(idx)
            disc_sum += p
        return pens_sum - disc_sum, ds

    # Precompute logV
    logV = array('d', [0.0]*M)
    for i in range(M):
        logV[i] = _log(V[i])

    # Phase 1: coordinate descent in log space
    log_a = [0.0] * N
    for iteration in range(30):
        if time.time() - t0 > 1.5:
            break
        for v in range(N):
            start = offsets[v]; end = offsets[v+1]
            if start == end:
                continue
            num = 0.0
            den = 0.0
            for j in range(start, end):
                w = W[adj_ci[j]]
                num += w * (logV[adj_ci[j]] - log_a[adj_ov[j]])
                den += w
            if den > 0:
                log_a[v] = clamp_log(num / den)

    sys.stderr.write(f"Phase 1 done: {time.time()-t0:.2f}s\n")
    sys.stderr.flush()

    a = [log_to_int(log_a[i]) for i in range(N)]
    best_penalty, best_ds_list = total_pen_with_discard(a)
    best_ds = set(best_ds_list)
    best_a = a[:]

    # Phase 2: re-solve without discarded
    for outer in range(3):
        if time.time() - t0 > 2.5:
            break
        log_a = [clamp_log(_log(max(1, ai))) for ai in a]
        for iteration in range(20):
            if time.time() - t0 > 2.5:
                break
            for v in range(N):
                start = offsets[v]; end = offsets[v+1]
                if start == end:
                    continue
                num = 0.0
                den = 0.0
                for j in range(start, end):
                    ci = adj_ci[j]
                    if ci in best_ds:
                        continue
                    w = W[ci]
                    num += w * (logV[ci] - log_a[adj_ov[j]])
                    den += w
                if den > 0:
                    log_a[v] = clamp_log(num / den)

        a = [log_to_int(log_a[i]) for i in range(N)]
        pen, ds_list = total_pen_with_discard(a)
        if pen < best_penalty - 1e-9:
            best_penalty = pen
            best_a = a[:]
            best_ds = set(ds_list)
        else:
            break

    sys.stderr.write(f"Phase 2 done: {time.time()-t0:.2f}s\n")
    sys.stderr.flush()

    del logV

    # Phase 3: local search
    a = best_a[:]

    for local_iter in range(15):
        if time.time() - t0 > 8.5:
            break
        improved = False

        for v in range(N):
            if time.time() - t0 > 8.5:
                break
            start = offsets[v]; end = offsets[v+1]
            if start == end:
                continue

            old_val = a[v]
            old_pen = 0.0
            for j in range(start, end):
                ci = adj_ci[j]; ov = adj_ov[j]
                old_pen += W[ci] * _abs(old_val * a[ov] - V[ci]) / V[ci]

            cands = []
            seen = {old_val}
            for delta in (-3,-2,-1,1,2,3):
                nv = old_val + delta
                if 1 <= nv <= 1000000000:
                    cands.append(nv); seen.add(nv)

            cnt = 0
            for j in range(start, end):
                if cnt >= 30: break
                cnt += 1
                ao = a[adj_ov[j]]
                if ao > 0:
                    ideal = V[adj_ci[j]] / ao
                    fl = max(1, int(ideal))
                    ce = fl + 1
                    if fl not in seen and fl <= 1000000000:
                        cands.append(fl); seen.add(fl)
                    if ce not in seen and ce <= 1000000000:
                        cands.append(ce); seen.add(ce)

            best_local = old_val
            best_local_pen = old_pen
            for nv in cands:
                p = 0.0
                for j in range(start, end):
                    ci = adj_ci[j]; ov = adj_ov[j]
                    p += W[ci] * _abs(nv * a[ov] - V[ci]) / V[ci]
                if p < best_local_pen - 1e-12:
                    best_local_pen = p
                    best_local = nv

            if best_local != old_val:
                a[v] = best_local
                improved = True

        sys.stderr.write(f"Local iter {local_iter} done: {time.time()-t0:.2f}s\n")
        sys.stderr.flush()
        if not improved:
            break

    pen, ds_list = total_pen_with_discard(a)
    if pen < best_penalty:
        best_a = a
        best_ds_list = ds_list

    # Output
    discard_1indexed = sorted(i+1 for i in best_ds_list)
    sys.stdout.write(' '.join(map(str, best_a)))
    sys.stdout.write('\n')
    sys.stdout.write(str(len(discard_1indexed)))
    if discard_1indexed:
        sys.stdout.write(' ')
        sys.stdout.write(' '.join(map(str, discard_1indexed)))
    sys.stdout.write('\n')
    sys.stderr.write(f"Done: {time.time()-t0:.2f}s penalty={pen:.2f}\n")

if __name__ == '__main__':
    try:
        main()
    except Exception as e:
        # Fallback: output all 1s and no discards
        sys.stderr.write(f"EXCEPTION: {e}\n")
        import traceback
        traceback.print_exc(file=sys.stderr)
        # Try to recover - re-read input for N
        try:
            sys.stdin.seek(0)
            line = open('/dev/stdin').readline() if False else ""
        except:
            pass
        # Can't recover without N, just exit
        sys.exit(1)
