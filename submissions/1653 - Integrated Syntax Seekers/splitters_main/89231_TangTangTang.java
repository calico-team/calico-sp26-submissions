import java.util.*;
import java.io.*;
import java.math.BigInteger;// using BigInteger cuz doubles have the precision of a fucking blind toddler

public class TangTangTang {  // tung tung reference?
    public static void main(String[] args) throws IOException {
        Scanner sc = new Scanner(System.in);// too lazy to use fast io

        

        int T = sc.nextInt();

        while (T-- > 0) {// typical ah loop
            int N = sc.nextInt();
            int M = sc.nextInt();
// the actual map of the grid
            char[][] grid = new char[N][M];
            for (int i = 0; i < N; i++) {
                grid[i] = sc.next().toCharArray();
            }
// probP is numerator, probQ is denominator
            BigInteger[][] probP = new BigInteger[N][M];
            BigInteger[][] probQ = new BigInteger[N][M];
            int[][] inDegree = new int[N][M];
// initialize everything to 0/1 
            for (int i = 0; i < N; i++) {
                for (int j = 0; j < M; j++) {
                    probP[i][j] = BigInteger.ZERO;
                    probQ[i][j] = BigInteger.ONE;
                }
            }

            probP[0][0] = BigInteger.ONE;
            probQ[0][0] = BigInteger.ONE;
//  find out how many things point to each tile 
            // so we know when it's safe to process it topological sort(i know big words trust)
            for (int r = 0; r < N; r++) {
                for (int c = 0; c < M; c++) {
                    char tile = grid[r][c];
                   // mf these split tiles
                    if (tile == 's' || tile == 'S') {
                        int[] dr = {-1, 1, 0, 0};
                        int[] dc = {0, 0, -1, 1};
                        char[] back = {'v', '^', '>', '<'};
                        for (int i = 0; i < 4; i++) {
                            ...
                            if (nr >= 0 && nr < N && nc >= 0 && nc < M) {
                                // if it's not a wall or pointing back at us, it's a valid exit
                                char nt = grid[nr][nc];
                                if (nt == 'x' || nt == 'X' || (nt != '.' && nt != 's' && nt != 'S' && nt != back[i])) {
                                    inDegree[nr][nc]++;
                                }
                            }
                        }
                       ...
                       // basic arrow tile
                        int nr = r, nc = c;
                        if (tile == '<') nc--; else if (tile == '>') nc++; else if (tile == '^') nr--; else nr++;
                        if (nr >= 0 && nr < N && nc >= 0 && nc < M) inDegree[nr][nc]++;
                    }
                }
            }
// Kahn's algorithm i think kinda maybe 
            Queue<int[]> q = new LinkedList<>();
            q.add(new int[]{0, 0});
           //final answer 
            BigInteger resP = BigInteger.ZERO;
            BigInteger resQ = BigInteger.ONE;

            while (!q.isEmpty()) {
                int[] curr = q.poll();
                int r = curr[0], c = curr[1];
                char tile = grid[r][c];
                BigInteger curP = probP[r][c];
                BigInteger curQ = probQ[r][c];
// if % hits wall it dies
...
// split the current prob equally among all valid neighbor exits
                    if (valid.size() > 0) {
                        BigInteger splitQ = curQ.multiply(BigInteger.valueOf(valid.size()));
                        for (int[] next : valid) {
                            int nr = next[0], nc = next[1];
                            // standard fraction addition
                            BigInteger nP = probP[nr][nc].multiply(splitQ).add(curP.multiply(probQ[nr][nc]));
                            BigInteger nQ = probQ[nr][nc].multiply(splitQ);
                           // simplify early and else the big i will eat my ram
                            BigInteger g = nP.gcd(nQ);
                            probP[nr][nc] = nP.divide(g);
                            probQ[nr][nc] = nQ.divide(g);
                            if (--inDegree[nr][nc] == 0) q.add(new int[]{nr, nc});
                        }
                    }
                } else {
                    // move with arrow Be the arrow-my friend says
                    int nr = r, nc = c;
                    if (tile == '<') nc--; else if (tile == '>') nc++; else if (tile == '^') nr--; else nr++;
// if we go off the board, we have escaped like the amazeing people we are
                    if (nr < 0 || nr >= N || nc < 0 || nc >= M) {
                        BigInteger nP = resP.multiply(curQ).add(curP.multiply(resQ));
                        BigInteger nQ = resQ.multiply(curQ);
                        BigInteger g = nP.gcd(nQ);
                        resP = nP.divide(g);
                        resQ = nQ.divide(g);
                    } else {
                        // otherwise idk just pass the prob to the next tile
                        BigInteger nP = probP[nr][nc].multiply(curQ).add(curP.multiply(probQ[nr][nc]));
                        BigInteger nQ = probQ[nr][nc].multiply(curQ);
                        BigInteger g = nP.gcd(nQ);
                        probP[nr][nc] = nP.divide(g);
                        probQ[nr][nc] = nQ.divide(g);
                        if (--inDegree[nr][nc] == 0) q.add(new int[]{nr, nc});
            ...
            System.out.println(resP + " " + resQ);
        }
    }
}
// i am so locked in i hope if it has a fast time litmit i am fried bro this confuses the fuck out of my brain
