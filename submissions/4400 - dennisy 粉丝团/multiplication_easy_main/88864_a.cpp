...
signed main() {
    ...
    vector<qy> rg(m);
    for (int i = 0; i < m; i++) {
        cin >> rg[i].r >> rg[i].c >> rg[i].v >> rg[i].w;
        rg[i].r--;
        rg[i].c--;
        a[rg[i].r] += sqrt(rg[i].v);
        tt[rg[i].r]++;
        a[rg[i].c] += sqrt(rg[i].v);
        tt[rg[i].c]++;
    }
    for (int i = 0; i < n; i++) {
        if (tt[i]) {
    ...
    for (int i = 0; i < m; i++) {
        double df = abs(1.0 * a[rg[i].r] * a[rg[i].c] - rg[i].v) / rg[i].v * rg[i].w;
        rem.pb({df, i});
    }
    sort(all(rem));
    reverse(all(rem));
