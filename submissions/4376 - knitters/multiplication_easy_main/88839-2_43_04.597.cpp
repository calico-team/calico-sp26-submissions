...
#define arstr vector<str>
#define matbool vector<arbool>
#define matstr vector<arstr>
...
void solve(){
	...
	for1(i, m)
		cin>>quer[i].r>>quer[i].c>>quer[i].v>>quer[i].w;
	
	arint id(m+1);
	for1(i, m)
		id[i] = i;
	
	sort(id.begin()+1, id.begin()+m+1, [](int a, int b){
		return quer[a].w < quer[b].w; 
		});
	sort(quer.begin()+1, quer.begin()+m+1, func);
	
	arll ans(n+1, 0);
	for1(i, m-d){
		if(ans[quer[i].r]) ans[quer[i].c] = quer[i].v/ans[quer[i].r];
		else if(ans[quer[i].c]) ans[quer[i].r] = quer[i].v/ans[quer[i].c];
		else{
			ans[quer[i].c] = ans[quer[i].r] = sqrt(quer[i].v);
		}
	}
