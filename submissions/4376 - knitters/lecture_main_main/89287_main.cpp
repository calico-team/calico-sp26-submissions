...
int n,m,k;
vii pref; 
pii check(int mid, int sz){
    fori(i, sz - mid-1)
        fori(j, sz - mid-1)
            if(pref[i+mid][j+mid] - pref[i-1][j+mid] - pref[i+mid][j-1] + pref[i-1][j-1] >= k) return {i, j};
    return {-1, -1};
}
...
int32_t main(){ios_base::sync_with_stdio(false);cin.tie(nullptr);cout.tie(nullptr);
    testcase{
        fin >> n >> m >> k;
        int sz = n + m + 2; 
        pref.assign(sz, vi(sz, 0));
        vs grid(n);
        fin >> grid;
        forni{
            form(j,m){
                if(grid[i][j] == '-'){
                    int x = i + j + 1, y = i - j + m + 1;
                    if(x < sz && y < sz) pref[x][y] = 1;
                }
            }
        }
        fori(i, sz - 1)
        ...
        while(l < r){
            int mid = l + (r - l) / 2;
            if(check(mid, sz).fs != -1)
                r = mid;
            else 
                l = mid + 1;
        }
        auto [x1, y1] = check(l, sz);
        int x2 = x1 + l, y2 = y1 + l, cnt = 0;
        forni{
            if(cnt >= k) break;
            form(j,m){
                if(cnt >= k) break;
