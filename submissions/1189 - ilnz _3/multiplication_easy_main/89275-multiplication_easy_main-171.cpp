...
signed main (signed argc, char **argv) {
	...
	for (auto i: vt) {
		tot[i.f.f] += sqrt((double)i.s.f)*(double)i.s.s;
		tot[i.f.s] += sqrt((double)i.s.f)*(double)i.s.s;
		num[i.f.f] += i.s.s;
		num[i.f.s] += i.s.s;
	}
	for (int i = 0; i < n; i++) arr[i] = num[i] ? round(tot[i]/num[i]) : 0;
	vector<int> cons(m);
	...
	sort(cons.begin(), cons.end(), [&](int a, int b) { return vt[a].s.s < vt[b].s.s;});
	vector<bool> bad(m, 0);
	for (int i = 0; i < k; i++) {
		bad[cons.back()] = true;
		cons.pop_back();
	...
	for (int i = 0; i < n; i++) {
		arr[i] = num[i] ? round(tot[i]/num[i]) : 0;
	}
