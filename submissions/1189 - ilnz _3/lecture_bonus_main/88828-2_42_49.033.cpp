...
bool f(int d){
	...
	for(auto i:v){a[i.f][i.s]++;a[i.f+d][i.s]--;a[i.f][i.s+d]--;a[i.f+d][i.s+d]++;}
	F(i,0,(n+m+5)){
		F(j,0,(n+m+5)){
...
void solve(){
	...
	vector<pii>x;
	F(i,0,po.size()){if(v[i].f<=p.f and p.f<v[i].f+hi and v[i].s<=p.s and p.s<v[i].s+hi and k>=1){int e=po[i].s.f+po[i].s.s-2+add;e/=2;x.emplace_back(e,po[i].s.s-e-1);}}
