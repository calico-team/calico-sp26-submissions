...
const int MAXX = (int) 200 * 2 * 1.5;
const int MAXY = (int) 200 * 2 * 1.5;

const int dx[] = {0, 1, 0, -1};
const int dy[] = {1, 0, -1, 0};

int Q;
int xm, ym, xg, yg, m[MAXX][MAXY];
...
void step(int m[MAXX][MAXY], queue<pair<int, int>> &q, bool type, int &nexp, int &mcnt) {
  nexp++;
  int sz = q.size();
  FOR(_, 0, sz) {
    ...
    FOR(d, 0, 4) {
      ...
      if (abs(newloc.f) > 200 || abs(newloc.s) > 200) {
        // cout << "!!!!!!!\n";
        continue;
      }
      // if (m.find(newloc) == m.end()) { // not claiemd yet
      if (m[newloc.f + 200][newloc.s + 200] == -1) {
        q.push(newloc);
        // m[newloc] = type;
        m[newloc.f + 200][newloc.s + 200] = type;
        if (!type) mcnt++;
      }
    }
  } 
