...
tcTUU> void DBG(const T& t, const U&... u) {
	...
	#define asrt(...) if (!(__VA_ARGS__)) cerr << "Line(" << __LINE__ << ") -> function(" \
...
void solve() {
	...
	// const int K = 15;
	// vi A = {rand() % K + 1, rand() % K + 1, rand() % K + 1, rand() % K + 1, rand() % K + 1, rand() % K + 1, rand() % K + 1};

	dbg(A);
	vi B = A;
	FOR(i, 1, N - 1) {
		A[i] = B[i - 1] + B[i] + B[i + 1] - max({B[i - 1], B[i], B[i + 1]}) - min({B[i - 1], B[i], B[i + 1]});
	}

	dbg(A);

	FOR(i, 1, N - 1) if (A[i] > A[i - 1] && A[i] > A[i + 1]) {
		A[i] = max(A[i - 1], A[i + 1]);
	}
	...
	FOR(i, 1, N - 1) if (A[i - 1] != A[i] || A[i] != A[i + 1]) {
		if (A[i] < B[i]) A[i] = max(A[i - 1], A[i + 1]);
		if (A[i] > B[i]) A[i] = min(A[i - 1], A[i + 1]);
	}
	...
	// 	F0R(i, N) {
	// 		ans += abs(C[i] - B[i]);
	// 		if (i + 1 < N) ans += abs(C[i] - C[i + 1]);
	// 		if (i + 1 < N) ans += abs(B[i] - B[i + 1]);
	// 	}
	...
	// dbg(A);
	// int bst = INF;
	// FOR(a, 1, K + 1) FOR(b, 1, K + 1) FOR(c, 1, K + 1) FOR(d, 1, K + 1) FOR(e, 1, K + 1) FOR(f, 1, K + 1) FOR(g, 1, K + 1) {
	// 	int res = comp({a, b, c, d, e, f, g});
	// 	if (ckmin(bst, res)) {
	// 		dbg(a, b, c, d, e, f, g, bst);
	// 	}
	// }
