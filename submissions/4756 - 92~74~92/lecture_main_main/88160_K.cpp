...
void solve() {
	...
	auto qry = [&](int r, int c, int R, int C) {
		return P[R + 1][C + 1] - P[r][C + 1] - P[R + 1][c] + P[r][c];
	};

	int ans = fstTrue(0, N + M + 1, [&](int k) {
		int amt = 0;
		F0R(r, N + M) F0R(c, N + M) {
			int rr = min(N + M - 1, r + k);
			int rc = min(N + M - 1, c + k);

			ckmax(amt, qry(r, c, rr, rc));
		}
	...
	vpi ANS;
	F0R(r, N + M) F0R(c, N + M) {
		int rr = min(N + M - 1, r + ans);
		int rc = min(N + M - 1, c + ans);

		if (qry(r, c, rr, rc) >= K && sz(ANS) == 0) {
			// dbg("FOUND");
			F0R(ri, N) F0R(ci, M) {
				int rx = ri + ci, cx = ri - ci + M;
				if (A[rx][cx] == 1 && r <= rx && rx <= rr && c <= cx && cx <= rc) {
					// dbg(r, rx, rc, c, cx, rc);
					ANS.pb(mp(ri, ci));
				}
			}
