...
void solve() {
	...
	auto H = [&](int rx, int cx) {
		return rx * 1LL * MOD + cx;
	};

	unordered_set<ll> seen, in; int ans = 0;
	pqg<AR<int, 4>> q; 
	q.push({0, 1, R, C}); in.ins(H(R, C));
	q.push({0, 0, r, c});

	while(sz(q) && sz(in) > 0) {
		auto [d, t, rx, cx] = q.top(); q.pop();
		dbg(d, t, rx, cx);
