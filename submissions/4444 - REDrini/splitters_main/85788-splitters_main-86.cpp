...
public:
	...
	const Integer orr(const Integer& other) const;
	const Integer andd(const Integer& other) const;
	const Integer xorr(const Integer& other) const;
	const Integer nott() const;
...
const Integer Integer::orr(const Integer& other) const
{
	Integer result = (*this);
...
const Integer Integer::andd(const Integer& other) const
{
	Integer result = 0;
...
const Integer Integer::xorr(const Integer& other) const
{
...
const Integer Integer::nott() const
{
	Integer result = *this;
...
Fraction operator +(const Fraction& a, const Fraction& b){
	Integer g = Fraction::gcd(a.den, b.den);
	Integer l = (a.den/g)*b.den;
	Integer na = a.num*(l/a.den);
	Integer nb = b.num*(l/b.den);
	return Fraction(na+nb, l);
}

Fraction operator /(const Fraction& a, int k){
	return Fraction(a.num, a.den*Integer(k));
}
...
array<int, 2> cal(int r, int c){
	if(s[r][c] == '^') return {r-1, c};
	if(s[r][c] == 'v') return {r+1, c};
	if(s[r][c] == '<') return {r, c-1};
	if(s[r][c] == '>') return {r, c+1};
}
...
void f(int r, int c){
	...
	if(s[r][c] == 'X'){
		...
		for(int d=0; d<4; d++){
			...
			if(s[nr][nc] == 'X') nxt.push_back({nr, nc});
			else if(s[nr][nc] == '^' || s[nr][nc] == 'v' || s[nr][nc] == '<' || s[nr][nc] == '>'){
				auto [tr, tc] = cal(nr, nc);
	...
	else if(s[r][c] == '^' || s[r][c] == 'v' || s[r][c] == '<' || s[r][c] == '>'){
		auto [nr, nc] = cal(r, c);
