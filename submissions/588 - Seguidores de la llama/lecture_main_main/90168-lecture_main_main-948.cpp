...
int main() {
    ...
    forn(_, t) {
        ...
        sum.init(), cnt.init();
        while (true) {
            vector<vi> queries(m + 1);
            bool flag = false;
            forn(i, C) if (hi[i] - lo[i] > 1) {
                queries[(lo[i] + hi[i]) / 2].pb(i);
                flag = true;
            }
            if (!flag) break;
            forn(aura, m + 1) {
                for (int i : queries[aura]) {
                    auto [x, y] = candidates[i];
                    ll currSum = sum.query(x - d + 1, y - d + 1, x + 1, y + 1);
                    ll currCnt = cnt.query(x - d + 1, y - d + 1, x + 1, y + 1);
                    if (currCnt >= k) {
                        hi[i] = aura;
                        sumRet[i] = currSum - aura * (currCnt - k);
                    } else {
                        lo[i] = aura;
                    }
                }
            ...
            forn(aura, m + 1) for (auto [x, y] : where[aura]) {
                sum.update(x, y, -aura);
                cnt.update(x, y, -1);
            }
        }
        ...
        int best = 0;
        forn(i, C) if (sumRet[i] < sumRet[best]) best = i;
        
        auto [x, y] = candidates[best];
        
        vector<pair<int, ii>> ret;
        forn(i, n) forn(j, m) if (s[i][j] == '-') {
            if (x - d < i - j + m && i - j + m <= x &&
                y - d < i + j && i + j <= y) {
