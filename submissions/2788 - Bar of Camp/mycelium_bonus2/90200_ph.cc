...
int main()
{
	...
	for (l=0;l<t;l++)
	{
		...
		y1-=y2;
		x2=0;y2=0;
		xx=max(min(x1,3000),-3000);
		yy=max(min(y1,3000),-3000);
		for (i=0;i<110000;i++)
			q[i].clear();
		q[0].push_back(make_pair(make_pair(x2+3000,y2+3000),7));
		q[(abs(xx-x1)+abs(yy-y1))*2].push_back(make_pair(make_pair(xx+3000,yy+3000),2));
		for (i=0;i<=6000;i++)
			a[i]=0;
		ans[0]=0;
		for (o=0;;o++)
