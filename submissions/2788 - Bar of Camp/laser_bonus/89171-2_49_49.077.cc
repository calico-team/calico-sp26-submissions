...
int main()
{
	...
	for (l=0;l<t;l++)
	{
		...
		for (i=0;i<nn;i++)
		{
			...
			tmp=gcd(abs((__int128)aa),abs((__int128)bb));
			...
			if (sol.second>0)
			{
				tmp=sol.second/abs((__int128)aa);
				sol.first-=tmp*abs((__int128)bb);
				sol.second-=tmp*abs((__int128)aa);
			}
			if (sol.second<0)
			{
				tmp=abs((__int128)sol.second)/abs((__int128)aa);
				sol.first+=tmp*abs((__int128)bb);
				sol.second+=tmp*abs((__int128)aa);
			}
			...
			while (tmpv<=base)
			{
				sol.first+=abs((__int128)bb);
				sol.second+=abs((__int128)aa);
				tmpv=make_pair(sol,make_pair(xx[i],yy[i]));
			}
