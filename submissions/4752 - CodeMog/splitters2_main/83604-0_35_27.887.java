...
public class Splitters2Submit {
    ...
    static String[] makeGrid(long p, int a, int b) {
        BigInteger[] twos = new BigInteger[a + 1];
        BigInteger[] threes = new BigInteger[b + 1];
        twos[0] = BigInteger.ONE;
        threes[0] = BigInteger.ONE;
        for (int i = 1; i <= a; i++) {
            twos[i] = twos[i - 1].shiftLeft(1);
        }
        for (int i = 1; i <= b; i++) {
            threes[i] = threes[i - 1].multiply(BigInteger.valueOf(3));
        }

        ArrayList<int[]> plan = new ArrayList<>();
        BigInteger cur = BigInteger.valueOf(p);
        int x = a;
        int y = b;

        while (x > 0) {
            BigInteger chunk = twos[x - 1].multiply(threes[y]);
            int bit = cur.compareTo(chunk) >= 0 ? 1 : 0;
            if (bit == 1) {
                cur = cur.subtract(chunk);
            }
            x--;
        ...
        while (y > 0) {
            BigInteger chunk = threes[y - 1];
            BigInteger[] qr = cur.divideAndRemainder(chunk);
            int digit = qr[0].intValueExact();
            cur = qr[1];
            y--;
