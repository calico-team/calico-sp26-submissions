...
long long x[100005], y[100005], stx, sty;
int xord[100005], yord[100005];
int anyord[100005];
...
void solve() {
    ...
    for(int i=0;i<n;i++) if(xord[i*p%n] == -1) {
        rnd++;
        xord[i*p%n] = i;
        yord[i*p%n] = cyy;
        cyy += q;
    ...
    for(int i=0;i<m;i++) if(anyord[cyy*i%m] == -1) anyord[cyy*i%m] = i;
    int incy = rnd*q%m; 
    pair<pair<int, int>, int> mn(pair<int, int>(100000000, 100000000), 0);
    for(int i=1;i<k;i++) {
        if(xord[x[i]] == -1) continue;
        y[i] = (y[i]-yord[x[i]]+m)%m;
        if(anyord[y[i]] == -1) continue;
