...
signed main(){
	...
	for(int i=1;i<=m;i++){
		...
		tot[aa[i][0]]+=(double)aa[i][3]/(double)aa[i][2];
		b[aa[i][1]]+=(double)aa[i][3]/(double)aa[i][2]*((double)sqrt((double)aa[i][2]));
		tot[aa[i][1]]+=(double)aa[i][3]/(double)aa[i][2];
	}
	for(int i=1;i<=n;i++)
	...
	sort(p+1,p+m+1);
	for(int i=m;i>=m-d+1;i--)
		ok[p[i].second]=1;
//	double error=0;
//	for(int i=1;i<=m;i++){
//		if(ok[i])
//			continue;
//		error+=(double)aa[i][3]/(double)aa[i][2]*(double)(abs((a[aa[i][0]]*a[aa[i][1]])-aa[i][2]));
//	}
//	cout<<error<<endl;
	for(int i=1;i<=m;i++){
		if(ok[i])
	...
	for(int i=1;i<=n;i++)
		a[i]=max(1ll,(int)floor(b[i]/tot[i]));
	for(int i=1;i<=m;i++){
		ok[i]=0;
		double ans=(double)aa[i][3]/(double)aa[i][2];
		ans=ans*(double)(abs((a[aa[i][0]]*a[aa[i][1]])-aa[i][2]));
