...
ll best_coordinate_value(int ind, const vector<ll>& a, const vector<char>& active) {
    ...
    candidates.push_back(max<ll>(1, low - 1));
    candidates.push_back(max<ll>(1, low));
    candidates.push_back(max<ll>(1, high));
    candidates.push_back(max<ll>(1, high + 1));
    ...
    for (int eid : adj[ind]) {
        ...
        for (int j = 0; j < (int)candidates.size(); j++) {
            costs[j] += edge_penalty(e, candidates[j], av);
        }
    }
...
vector<ll> initial_log_smooth(int rounds = 12) {
...
void try_run(vector<ll> a, int outerIters = 12) {
    ...
    for (int it = 0; it < outerIters; it++) {
        ...
        if (it < 3) shuffle(order.begin(), order.end(), rng);
