...
def solve_linear_congruence(a, b, m):
    """Solve a*t ≡ b (mod m). Returns (t0, period) or None if no solution."""
    g = gcd(a, m)
    if b % g != 0:
        return None
    # Divide through by g to get a'*t ≡ b' (mod m')
    a_, b_, m_ = a // g, (b // g) % (m // g), m // g
...
def crt(r1, m1, r2, m2):
    ...
    Returns (t0, lcm) or None if no solution.
    """
...
def solve(K, N, M, P, Q, X, Y):
    ...
    for i in range(K):
        ...
        # Combine both congruences via CRT
        ...
        # t=0 is the starting position — laser only hits it after a full cycle
        if t == 0:
            t = period
