...
void solve()  {
    ...
    queue<pair<pair<int,int>,pair<pair<int,int>,pair<int,int>>>> q2;
    q2.push({{0,0},{0,0},{-1,-1}});
    while (!q2.empty())  {
        int curx = q2.front().first.first;
        int cury = q2.front().first.second;
        int prevx = q2.front().second.second.first;
        int prevy = q2.front().second.second.second;
        q2.pop();
        if (grid[curx][cury] == 'S')  {
            int cnt = 0;
            if (valid.find(grid[curx+1][cury]) != valid.end())  indeg[curx+1][cury]++;
            if (valid.find(grid[curx-1][cury]) != valid.end())  indeg[curx-1][cury]++;
            if (valid.find(grid[curx][cury+1]) != valid.end())  indeg[curx][cury+1]++;
            if (valid.find(grid[curx][cury-1]) != valid.end())  indeg[curx][cury-1]++;
            //cout << "WEOUGBWEUG " << curx << ' ' << cury << ' ' << cnt << endl;
            if (valid.find(grid[curx+1][cury]) != valid.end())  {
                if (!(curx+1 == prevx && cury == prevy))  {
                    q2.push({{curx+1, cury}, {{0,0}, {curx, cury}}});
                } 
            }
            if (valid.find(grid[curx-1][cury]) != valid.end())  {
                if (!(curx-1 == prevx && cury == prevy))  {
            ...
            if (valid.find(grid[curx][cury+1]) != valid.end())  {
                if (!(curx == prevx && cury+1 == prevy))  {
                    q2.push({{curx, cury+1}, {{0,0}, {curx, cury}}});
                }
            }
            if (valid.find(grid[curx][cury-1]) != valid.end())  {
                if (!(curx == prevx && cury-1 == prevy))  {
