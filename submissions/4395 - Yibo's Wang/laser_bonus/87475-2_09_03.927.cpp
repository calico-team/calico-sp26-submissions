...
pair<int,int> spit(int a, int b, int c)  { 
    b = ((b%c)+c)%c;
    b %= c;
    int g = gcd(a, c);
    if (b % g != 0)  {
...
void solve()  {
    ...
    for (int i = 0; i < k; i++)  {
        ...
        if ((r2-r1) % gcd(m1, m2) == 0)  {
            ...
            int g = gcd(m1, m2);
            int mod2 = m2 / g;
            int ans = ((r1+m1 *(((r2-r1)/g%mod2+mod2)% mod2 * modInverse(m1/g, mod2) %mod2) % clcm)) %clcm;
            //cout << "GWGE " << i << ' ' << ans << ' ' << clcm<< endl;
            ans = ((ans%clcm)+clcm)%clcm;
            if (ans == 0)  {
                ans = clcm;
            }
            if (ans < curmin)  {
