...
int main() {
    ...
    V<double> best_x(n, 0.0);
    V<bool> best_disc(m, false);
    double best_penalty = 1e18;

    mt19937 rng(67676767);
    V<double> wf(m);
    for (int i = 0; i < m; i++) wf[i] = (double)w[i];
    ...
    for (int trial = 0; trial < 5; trial++) {
        ...
        if (trial == 0) {
            ...
            V<double> weights(wf.begin(), wf.end());
            for (int i = 0; i < d; i++) {
        ...
        for (int k = 0; k < m; k++) {
            ...
            int rr = r[k], cc = c[k];
            double prod = exp(x[rr] + x[cc]);
            total += (double)w[k] * fabs(prod - v[k]) / fabs(v[k]);
        }
    ...
    V<int> disc_final;
    for (int k = 0; k < m; k++) if (best_disc[k]) disc_final.push_back(k + 1);
