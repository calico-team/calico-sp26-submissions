...
void pb(ll x,ll y){
	if(jdz(x-x2)+jdz(y-y2)>=900&&!zzj(x,y))return;
	if(zzj2(x,y))return;
	if(x-1>=0&&!zy[x-1][y]){
		vx.push(x-1);
		vy.push(y);
...
int main(){
	...
	while(T--){
		...
		ll t2=0;
		if(jdz(x1-x2)+jdz(y2-y3)>1500){
			//t2=jdz(x1-x2)+jdz(y2-y3)-900;
			ll jbs=jdz(x1-x2)+jdz(y2-y3)-1500;
			ll jx=jdz(x1-x2);
			jx=jbs*jx/(jdz(x1-x2)+jdz(y2-y3));
			ll jy=jdz(y3-y2);
			jy=jbs*jy/(jdz(x1-x2)+jdz(y2-y3));
			t2=jx+jy;
			if(x1<x2)x1+=jx;
			else x1-=jx;
			if(y3<y2)y3+=jy;
