def solve(N: int, S1: list[str], S2: list[str]) -> tuple[int, int]:
    ...
    """
    # Maximum: product of hashtag counts at each depth
    max_vol = 0
    for i in range(N):
        count_s1 = S1[i].count('#')
        count_s2 = S2[i].count('#')
    ...
    # Minimum: for each z-layer, use greedy set cover
    min_vol = 0
    
    for z in range(N):
        s1_rows = set(y for y in range(N) if S1[z][y] == '#')
        s2_cols = set(x for x in range(N) if S2[z][x] == '#')
        
        if not s1_rows or not s2_cols:
            continue
        
     
        uncovered_s1 = s1_rows.copy()
        uncovered_s2 = s2_cols.copy()
