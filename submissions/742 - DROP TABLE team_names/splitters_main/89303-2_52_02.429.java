...
class splitters_template {
    public static long total = 1, collected = 0;
        ...
        while (true) {
            ...
            if (grid[y][x] == 'S') {  
                  List<Dir> dirs = getValidDirs(x, y);
                    long N = dirs.size();
                    long sumNum = 0, sumDen = 1;

                    for (Dir d : dirs) {
                        long[] branch = func(x, y, d);
                        long tempNum = (sumNum * branch[1] + branch[0] * sumDen),
                            tempDen = (sumDen * branch[1]);
                        long gcd = gcd(Math.abs(tempNum), tempDen);
                        sumNum = tempNum / gcd;
                        sumDen = tempDen / gcd;
                    }
                    
                    long finalDen = sumDen * N;
                    long gcd = gcd(Math.abs(sumNum), finalDen);
