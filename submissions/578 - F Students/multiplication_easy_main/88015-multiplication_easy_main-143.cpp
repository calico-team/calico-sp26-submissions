...
void solve() {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    sum[c[i].u] += s, sum[c[i].v] += s;
    ++cnt[c[i].u], ++cnt[c[i].v];
  }
  vector<ll> a(n, 1);
  for (int i = 0; i < n; ++i) {
    if (!cnt[i]) continue;
    a[i] = max<ll>(1, round(sum[i] / cnt[i]));
  }
  for (int iii = 0; iii < 3; ++iii) {
    vector<double> z(n, 0.0);
    for (int i = 0; i < m; ++i) {
      z[c[i].u] += c[i].val / a[c[i].v];
      z[c[i].v] += c[i].val / a[c[i].u];
    }
    for (int i = 0; i < n; ++i) {
      if (!cnt[i]) continue;
      a[i] = max<ll>(1, round(z[i] / cnt[i]));
    }
  }
  vector<pair<double, int>> nscore(m);
  for (int i = 0; i < m; ++i) {
    double real = a[c[i].u] * a[c[i].v];
