...
vector<pair<int,int>> fract;
void travel(int x, int y, int numer, int denom, int prevx, int prevy)
{
    ...
    else if (factory[x][y] == 'S')
    {
        ...
        if (factory[x][y-1] == 'v' || factory[x][y-1] == '.') poss[0] = false;
        if (factory[x-1][y] == '>' || factory[x-1][y] == '.') poss[1] = false;
        if (factory[x][y+1] == '^' || factory[x][y+1] == '.') poss[2] = false;
        if (factory[x+1][y] == '<' || factory[x+1][y] == '.') poss[3] = false;
        int cnt = 0; for (int i = 0; i < 4; i++) cnt += poss[i];
...
int main()
{
    ...
    {
        int n, m; cin >> n >> m;
        factory.clear(); factory.resize(n); fract.clear(); fract.push_back({0,1});
        for (int i = 0; i < n; i++) cin >> factory[i];
        travel(0,0,1,1,-1,-1);
        ll denom = 1, numer = 0;
        for (pair<int,int> p1 : fract)
        {
