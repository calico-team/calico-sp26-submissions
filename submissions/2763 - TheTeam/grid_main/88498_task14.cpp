...
void solve()
{
    ...
    ll dp[N + 1][M]; 
    ll reconstruct[N + 1][M + 1];
    //dp is min cost 
    for (int k = 0; k < M; k++)
    {
        dp[1][k] = llabs(vals[k] - arr[1]);
    }
    for (int i = 2; i <= N; i++)
    {
        vector<ll> pref_val(M); vector<ll> pref_idx(M);

        pref_val[0] = dp[i - 1][0] - vals[0];
        pref_idx[0] = 0;

        for (int j = 1; j < M; j++)
        {
            ll d = dp[i - 1][j] - vals[j];
            if (d < pref_val[j - 1])
            {
                pref_val[j] = d;
                pref_idx[j] = j;
        ...
        suffix_val[M - 1] = dp[i - 1][M - 1] + vals[M - 1];
        suffix_idx[M - 1] = M - 1;
        for (int j = M -2; j >= 0; j--)
        {
            ll d = dp[i - 1][j] + vals[j];
        ...
        for (int j = 0; j < M; j++)
        {
            ...
            if (left <= right)
            {
                dp[i][j] = llabs(vals[j] - arr[i]) + left;
                reconstruct[i][j] = pref_idx[j];
            }
            else
            {
                dp[i][j] = llabs(vals[j] - arr[i]) + right;
