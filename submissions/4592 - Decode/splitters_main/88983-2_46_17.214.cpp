...
//		cout << q.size() << ' ' << "AT: " << x << ' ' << y << " w/ " << factory[x][y] <<'\n';
		assert(factory[x][y] != '.');
			...
			case 'S':
				{
					...
					if(factory[x+1][y] != '.' && factory[x+1][y] != '^') tosplit.push_back({x+1, y});
					if(factory[x-1][y] != '.' && factory[x-1][y] != 'v') tosplit.push_back({x-1, y});
					if(factory[x][y+1] != '.' && factory[x][y+1] != '<') tosplit.push_back({x, y+1});
					if(factory[x][y-1] != '.' && factory[x][y-1] != '>') tosplit.push_back({x, y-1});

					assert(tosplit.size() >= 2);
