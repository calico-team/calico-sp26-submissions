...
pair<long long, long long> calc(pair<long long, long long> i, pair<long long, long long> c){
    ...
    while(true){
        ...
        if(gr[c.first][c.second] == 'S'){
            int p = 0;
            for(int x = 0; x < 4; x++){
                if(gr[c.first + d[x].first][c.second + d[x].second] != '.' && make_pair(c.first + d[x].first, c.second + d[x].second) != prv){
                    ++p;
                }
            }
            ...
            for(int x = 0; x < 4; x++){
                if(gr[c.first + d[x].first][c.second + d[x].second] != '.' && make_pair(c.first + d[x].first, c.second + d[x].second) != prv){
                    pair<long long, long long> cr = calc(make_pair(i.first, i.second * p), make_pair(c.first + d[x].first, c.second + d[x].second));
