...
public class Main {
    public static void main(String[] args) throws Exception {
        ...
        for (int i = 0; i < T; i++) {
            ...
            for (int j = 1; j < N - 1; j++) {
                if ((arr2[j - 1] <= arr2[j] && arr1[j] >= arr2[j])) {
                    if (arr1[j + 1] < arr2[j]) {
                ...
                } else if (arr2[j - 1] >= arr2[j] && arr1[j] <= arr2[j]) {
                    if (arr1[j + 1] < arr2[j]) {
                        int maxDecrease = Math.max(arr1[j], arr1[j + 1]);
                        arr2[j] = maxDecrease;
                    } else {
                        int maxIncrease = Math.min(arr2[j - 1], arr1[j + 1]);
                        arr2[j] = maxIncrease;
                    }
                }
