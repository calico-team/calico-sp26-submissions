...
public class P5 { 
    ...
    static void solve() throws Exception {
        ...
        // for (int t=0; t<NUMT; t++) {
            ...
            //     var a = new int[n];
            //     for (int i=0; i<n; i++) {
                //         a[i] = r.nextInt(1, MAXN);
                //     }
                //     int ans = solve(n, a);
                //     int bf = bruteForce(n, a);
                //     if (ans != bf) {
                    //         System.out.println("wrong " + Arrays.toString(a) + ", ans = " + ans + ", bf = " + bf);
                    //     }
                    // }
                    
                    
        int n = io.nextInt();
        var a = io.nextArrayInt(n);
        var b = solve(n, a);
        for (int i=0; i<n; i++) {
            if (i>0) {
                System.out.print(" ");
    ...
    static void recurse(int[] a, int[] b, int i) {
        
        if (i >= b.length) {
            bestScore = Math.min(bestScore, score(a, b));
            return;
        }
