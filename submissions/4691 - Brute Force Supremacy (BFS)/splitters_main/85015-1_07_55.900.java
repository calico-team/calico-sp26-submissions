...
public class P9 { 
    ...
    static char[] invalid = {'v', '<', '^', '>'};
    static Fraction[][] memo;

    static Fraction solve(int i, int j) {
        if (i < 0 || i >= n || j < 0 || j >= m) {
        ...
        if (c == 'v') {
            ...
            return memo[i][j] = new Fraction(0,1);
        } else if (c == 'S') {
            int ct = 0;
            for (int k=0; k<4; k++) {
                int ni = i+di[k];
                int nj = j+dj[k];
                if (ni<0 || ni>=n || nj<0 || nj>=m || g[ni][nj] == invalid[k] || g[ni][nj] == '.') {
                    continue;
                }
                ct++;
            }
            //System.out.println("splitter " + i + ", " + j + " - ct = " + ct);
            Fraction ans = new Fraction(0, 1);
            var p = new Fraction(1, ct);
            ...
            for (int k=0; k<4; k++) {
                ...
                ans = ans.add(p.multiply(solve(ni, nj)));
            }
            return memo[i][j] = ans;
        } 

        //System.out.println("gave up at " + i + ", " + j);
        return null;
    }
    ...
    static class Fraction implements Comparable<Fraction> {
        ...
        public Fraction(long x, long y) {
            if (y < 0) { // move sign to the numerator
                x *= -1;
                y *= -1;
            }
            if (y == 0) { // normalize division by zero
                x = 1;
            } else {
                long f = gcd(Math.abs(x), y);
                x /= f;
                y /= f;
            }
            this.x = x;
        ...
        Fraction add(Fraction b) { return new Fraction(this.y * b.x + this.x * b.y, this.y * b.y); }
        Fraction subtract(Fraction b) { return new Fraction(this.x * b.y - this.y * b.x, this.y * b.y); }
        Fraction multiply(Fraction b) { return new Fraction(this.x * b.x, this.y * b.y); }
        Fraction divide(Fraction b) {  return new Fraction(this.x * b.y, this.y * b.x); }
        Fraction scale(long s) { return new Fraction(s*this.x, this.y); }

        public String toString() { return "(" + x + "/" + y + ")"; }
        public int compareTo(Fraction o) { return Long.compare(this.x * o.y, this.y * o.x); }
        public int hashCode() { return Long.hashCode(1_000_000_007 * x + y); }
