...
struct DSU {
    vi p, s;
    ll num, mx2;
    void init(ll n, map<ll, ll>& info) {
        p = vi(n + 1, -1);
        s = vi(n + 1);
        FOR(i, 1, n + 1) s[i] = info[i];
        num = 0;
        mx2 = num;
    }
    ll find(ll x) {
    ...
    void reset(vi& res, map<ll, ll>& info) {
        for (auto x : res) {
            p[x] = -1;
            s[x] = info[x];
        }
...
pii sub(pii a, pii b) {
    ll num = (a.ff * b.ss) - (a.ss * b.ff);
    ll den = a.ss * b.ss;
    if (!num) return {0, 1};
    ll c = gcd(abs(num), abs(den));
    return {num / c, den / c};
}

pii mult(pii a, pii b) {
    ll num = (a.ff * b.ff), dem = (a.ss * b.ss);
    ll c = gcd(num, dem);
    return {num / c, dem / c};
}
...
void solve() {
    ...
    while (!q.empty()) {
        auto curr = q.front(); q.pop();
        auto x = curr.ff.ff, y = curr.ff.ss;
        pii frac = curr.ss;
        ll c = get(v[x][y]);
        if (v[x][y] == 'X' || v[x][y] == '.') { ans = sub(ans, frac); continue; }
        if (c != -1) {
            ll nx = x + dx[c];
            ll ny = y + dy[c];
            if (nx >= 0 && nx < n && ny >= 0 && ny < m) q.push({{nx, ny}, frac});
            continue;
        }
        else if (v[x][y] == 'S') {
            vpii valid;
            FOR(i, 0, 4) {
                ...
                if (nx >= 0 && nx < n && ny >= 0 && ny < m) {
                    ...
                    ll nc = get(v[nx][ny]);
                    if ((nc == -1) || (nx + dx[nc] == x) && (ny + dy[nc] == y)) continue;
                    valid.pb({nx, ny});

                }
            }
            pii frac1 = mult(frac, {1LL, (ll)szof(valid)});
            for (auto [nx, ny] : valid) q.push({{nx, ny}, frac1});
        }
    }
    ll GCD = gcd(ans.ff, ans.ss);
    cout << ans.ff / GCD << " " << ans.ss / GCD << "\n";
    return;
}
