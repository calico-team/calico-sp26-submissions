...
vvi build(vvi& v) {
    ...
    FOR(i, 0, n) {
        FOR(j, 0, m) {
            bool ii = (i > 0), jj = (j > 0);
            psum[i][j] = v[i][j];
            if (ii) psum[i][j] += psum[i - 1][j];
            if (jj) psum[i][j] += psum[i][j - 1];
            if (ii && jj) psum[i][j] -= psum[i - 1][j - 1];
        }
    }
    return psum;
}

ll query(vvi& psum, ll x1, ll y1, ll x2, ll y2) {
    ll ret = psum[x2][y2];
    bool x11 = (x1 > 0), y11 = (y1 > 0);
    if (x11) ret -= psum[x1 - 1][y2];
    if (y11) ret -= psum[x2][y1 - 1];
    if (x11 && y11) ret += psum[x1 - 1][y1 - 1];
    return ret;
}

ll x11, y11, x22, y22;

bool check(ll mid, vvi& grid, vvi& psum) {
    FOR(i, 0, n - mid) {
        FOR(j, 0, m - mid) {
            if (query(psum, i, j, i + mid, j + mid) >= k) { x11 = i, y11 = j, x22 = i + mid, y22 = j + mid; return 1; }
        }
    }
    return 0;
}
...
void solve() {
    //optimal townhall-ish?
    //WAIT IVE PLAYED THESE GAMES B4 
    ll nn, mm;
    cin >> nn >> mm >> k;
    n = mx;
    m = n;
    ...
    for (auto [x, y] : pts) {
        ll nx = x + y, ny = x - y + mm;
        if (nx >= x11 && nx <= x22 && ny >= y11 && ny <= y22) {
            cout << x << " " << y << "\n";
            ++cnt;
            if (cnt == k) break;
        }
