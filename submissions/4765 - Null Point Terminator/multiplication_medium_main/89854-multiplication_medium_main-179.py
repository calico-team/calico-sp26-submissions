...
def main():
    ...
    for cd in range(10000):
        ...
        for j in range(n):
            ...
            seen=set()
            cands=[]
            for v in(f-2,f-1,f,c,c+1,old-1,old+1):
    ...
    #restart from different init if time allows (only for small/medium inputs)
    def total_pen(av):
        s=0.0
        for i in range(m):
            vi=V[i]
            if vi:s+=W[i]*abs(av[R[i]]*av[C[i]]-vi)/vi
            else:s+=W[i]*av[R[i]]*av[C[i]]
        return s

    best_a=a[:]
    if m<=50000:
        best_score=total_pen(a)
    else:
        best_score=1e18

    if m<=50000 and time.time()-t0<7.0:
        #second start: all values set to round(sqrt(median V))
        import math
        med_v=sorted(V)[m//2]
        sv=max(1,min(1000000000,int(math.isqrt(med_v)+0.5)))
        a2=[sv]*n
        active2=bytearray(b'\x01'*m)
        pens2=array('d',[0.0]*m)
        def upd2():
            for i in range(m):
                prod=a2[R[i]]*a2[C[i]];vi=V[i]
                pens2[i]=W[i]*abs(prod-vi)/vi if vi else W[i]*prod
            if d>0:
                ord2=sorted(range(m),key=pens2.__getitem__,reverse=True)
                active2[:]=b'\x01'*m
                for k in range(min(d,m)):active2[ord2[k]]=0
        upd2()
        for cd2 in range(10000):
            if time.time()-t0>9.0:break
            chg2=False
            for j in range(n):
                if j%200==0 and time.time()-t0>9.0:break
                ...
                items2=[]
                for ci,oi in zip(ci_sl,oi_sl):
                    if not active2[ci]:continue
                    other=a2[oi];vi=V[ci]
                    if other<=0 or vi==0:continue
                    items2.append((vi/other,W[ci]*other/vi))
                if not items2:continue
                items2.sort()
                tw=0.0
                for _,wt in items2:tw+=wt
                hf=tw*0.5;cm=0.0;md=items2[0][0]
                for t2,wt in items2:
                    cm+=wt
                    if cm>=hf:md=t2;break
                f2=int(md)
                if f2<1:f2=1
                if f2>1000000000:f2=1000000000
                c2=f2+1 if f2<1000000000 else f2
                old2=a2[j]
                seen2=set();cands2=[]
                for v in(f2-2,f2-1,f2,c2,c2+1,old2-1,old2+1):
                    if 1<=v<=1000000000 and v!=old2 and v not in seen2:
                        cands2.append(v);seen2.add(v)
                ...
                def pen2(val):
                    ...
                    for ci,oi in zip(ci_sl,oi_sl):
                        if active2[ci]:
                            vi=V[ci]
                            if vi:p+=W[ci]*abs(val*a2[oi]-vi)/vi
                            else:p+=W[ci]*val*a2[oi]
                    return p
                bp2=pen2(old2);b2=old2
                for cand in cands2:
                    p=pen2(cand)
                    if p<bp2:bp2=p;b2=cand
                if b2!=old2:a2[j]=b2;chg2=True
            if not chg2:break
            if time.time()-t0>9.0:break
            upd2()
        sc2=total_pen(a2)
        if sc2<best_score:best_a=a2;best_score=sc2
