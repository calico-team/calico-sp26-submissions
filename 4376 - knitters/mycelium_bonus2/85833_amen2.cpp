...
int dx[4] = {1, -1, 0, 0};
int dy[4] = {0, 0, 1, -1};
    uint8_t grid[mxn][5000];
    int qry(int x,int y){
        return (grid[x][y >> 2] >> ((y & 3) << 1)) & 3;
    }
    inline void upd(int x, int y, int val) {
        int shift = (y & 3) << 1;
        grid[x][y >> 2] = (grid[x][y >> 2] & ~(3 << shift)) | (val << shift);
    }
int32_t main(){ios_base::sync_with_stdio(false);cin.tie(nullptr);cout.tie(nullptr);
    testcase{
        ...
        while(!b.empty()){
            ...
            tick = min(((tick/2)+1)*2, ((tick/7)+1)*7);
        }
        fout << cnt << endl;
    }
}