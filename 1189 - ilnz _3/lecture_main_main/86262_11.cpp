...
void solve(){
	...
	F(i,0,n)F(j,0,m){char c;cin>>c;if(c=='-')v.emplace_back(i-j,i+j),add=max(add,-(i+j)),g.emplace_back(i,j);}
	int lo=0,hi=2000;
	while(lo<hi-1){
		int m=(lo+hi)>>1;
		if(f(m))hi=m;
	...
	dbg(v);
	F(i,0,v.size()){
		dbg(v[i],max(abs(v[i].f-p.f),abs(v[i].s-p.s)),k,hi);
		if(max(abs(v[i].f-p.f),abs(v[i].s-p.s))<=hi and k>=1){cout<<g[i].f<<" "<<g[i].s<<"\n";k--;}
	}
}
