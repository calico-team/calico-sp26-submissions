...
void solve() {
    //dang these are some insane intros man :D 
    ll gs, ge, ms, me;
    cin >> ms >> me >> gs >> ge;
    ll ans = 0;
    FOR(i, ms - mx, ms + mx + 10) {
        FOR(j, me - mx, me + mx + 10) {
            ll dist1 = abs(i - gs) + abs(j - ge);
            ll dist2 = abs(i - ms) + abs(j - me);
            if (7 * (dist2 - 1) < 2 * (dist1 - 1)) ++ans;
            //YOOYOYOY I GOT IT!!!!!!
        }
    }
    cout << ans << "\n";
