...
// a b c
// a e c 
// |e-a| + |e-c| + |e-b| -> max(min(a, c), min(max(a, c), b))

struct Solver {
int n;
vi arr, lo, hi;
vi res;
Solver(int n): n{n}, arr(n), lo(n), hi(n), res(n) {}
void solve() {
	forR(i, n) cin >> arr[i];
	forR(i, n) {
		lo[i] = i > 0 ? min(arr[i], arr[i-1]) : arr[i];
		hi[i] = i > 0 ? max(arr[i], arr[i-1]) : arr[i];
	}

	res[n-1] = arr[n-1];
	for(int i = n-2; i >= 0; --i) {
		if(res[i+1] > hi[i]) res[i] = hi[i];
		else if(res[i+1] < lo[i]) res[i] = lo[i];
		else res[i] = res[i+1];
	}
	forR(i, n) cout << res[i] << " \n"[i == n-1];
}
...
// edges should always be equal to top row
// second should be between A's second and A's first
signed main() {
    boost();
	int t; cin >> t;
	while(t--) {
