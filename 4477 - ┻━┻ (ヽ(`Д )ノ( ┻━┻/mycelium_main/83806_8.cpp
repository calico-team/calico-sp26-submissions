...
void tick(int ct) {
	forR(i, MN) forR(j, MN) nex[i][j] = cur[i][j];
	if(ct % 2 == 0) {
		forR(i, MN) forR(j, MN) {
			if(cur[i][j] == 1) {
				if(nex[i-1][j] == 0) nex[i-1][j] = 1;
				if(nex[i][j-1] == 0) nex[i][j-1] = 1;
				if(nex[i+1][j] == 0) nex[i+1][j] = 1;
				if(nex[i][j+1] == 0) nex[i][j+1] = 1;
			}
		}
	...
	if(ct % 7 == 0) {
		forR(i, MN) forR(j, MN) {
			if(cur[i][j] == 2) {
				if(nex[i-1][j] == 0) nex[i-1][j] = 2;
				if(nex[i][j-1] == 0) nex[i][j-1] = 2;
				if(nex[i+1][j] == 0) nex[i+1][j] = 2;
				if(nex[i][j+1] == 0) nex[i][j+1] = 2;
			}
		}
	}
	forR(i, MN) forR(j, MN) cur[i][j] = nex[i][j];
}
void solve() {
	int xg, yg, xm, ym;
	cin >> xg >> yg >> xm >> ym;
