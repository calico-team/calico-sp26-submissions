...
vector<int> runUntilSplitter(int x, int y, vector<string> factory) {
    ...
    if (factory[i][j] == '^') {
        ...
        return runUntilSplitter(j, i - 1, factory);
    }
    if (factory[i][j] == '>') {
        if (j == factory[0].size() - 1) {
            return {1, 1};
        }
        return runUntilSplitter(j + 1, i, factory);
    }
    if (factory[i][j] == 'v') {
        if (i == factory.size() - 1) {
            return {1, 1};
...
pair<long long, long long> solve(int N, int M, vector<string>& factory) {
    ...
    long long num = 1;
    long long denom = 1;
    vector<int> splitterSeq;
    vector<vector<int>> splitterNums;
    map<vector<int>, int> splitterID;
    ...
    for (int i = 0; i < N; i++) {
        for (int j = 0; j < N; j++) {
            if (factory[i][j] == 'S') {
                ...
                splitterNums.push_back(nums);
                splitterID[{i,j}] = splitterNums.size() - 1;
            }
        }
    }
