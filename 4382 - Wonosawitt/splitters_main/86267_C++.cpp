...
void solve() {
    ...
    for(int i=0;i<n;i++)
        for(int j=0;j<m;j++) {
            int cur = hash({i, j});
            if(s[i][j] == 'v') {
                if(i+1==n) adj[cur].Pb(hash({n, 0}));
                else adj[cur].Pb(hash({i+1, j}));
            } else if(s[i][j] == '^') {
                if(i==0) adj[cur].Pb(hash({n, 0}));
                else adj[cur].Pb(hash({i-1, j}));
            } else if(s[i][j] == '>') {
                if(j+1==m) adj[cur].Pb(hash({n, 0}));
                else adj[cur].Pb(hash({i, j+1}));
            } else if(s[i][j] == '<'){
                if(j==0) adj[cur].Pb(hash({n, 0}));
                else adj[cur].Pb(hash({i, j-1}));
            } else if(s[i][j] == 'S') {
                if(0<=i-1 && (s[i-1][j]=='^' || s[i-1][j]=='X')) adj[cur].Pb(hash({i-1, j}));
                if(i+1<n && (s[i+1][j]=='v' || s[i+1][j]=='X')) adj[cur].Pb(hash({i+1, j}));
                if(0<=j-1 && (s[i][j-1]=='<' || s[i][j-1]=='X')) adj[cur].Pb(hash({i, j-1}));
                if(j+1<n && (s[i][j+1]=='>' || s[i][j+1]=='X')) adj[cur].Pb(hash({i, j+1}));
            }
        }
