...
constexpr int MOD = 998244353, N = 2e5 + 67, MAX = 1e9;
...
void solve() {
  ...
  if (d == m) {
    for (int i = 0; i < n; ++i) cout << 1 << _;
    cout << endl << m << _;
    for (int i = 1; i <= m; ++i) cout << i << _;
    cout << endl;
    return;
  }
  ld BEST = DBL_MAX;
  ...
  while (time() < 9000) {
    ...
    for (int i = 0; i < n; ++i) {
      ...
      sort(all(s));
      a[i] = max<ll>(1, min<ll>(MAX, round(s[s.size() >> 1])));
      if (!f) {
        double rand = uniform_real_distribution<double>(0.4, 2.5)(rng);
        a[i] = max<ll>(1, min<ll>(MAX, round(a[i] * rand)));
      }
    }
    f = 0;
    vector<bool> on(m, 1);
      ...
      for (int iii = 0; iii < 20; ++iii) {
        ...
        for (int i : order) {
          ...
          for (int j : adj[i]) {
            ...
            ld x = (ld)c[j].val / a[v],
               y = dolog ? c[j].w : c[j].w * a[v] / c[j].val;
            tr.push_back({x, y}), tot += y;
          }
          if (tr.empty()) continue;
          sort(all(tr));
          ld run = 0, mid = tr.back().x;
          for (auto& t : tr) {
            run += t.y;
            if (run >= tot / 2) {
              mid = t.x;
          ...
          auto s = [&](ll x) {
            ...
            for (int j : adj[i]) {
              ...
              ll p = x * a[v];
              if (dolog)
                y += c[j].w * abs(log(p) - log(c[j].val));
              else {
                ld diff = p > c[j].val ? p - c[j].val : c[j].val - p;
                ld err = c[j].w * diff / c[j].val;
                y += err;
              }
            }
            return y;
          };
          ll tr1 = max<ll>(1, min<ll>(MAX, round(mid)));
          ll besti = -1;
          ld best = LDBL_MAX;
          for (ll idx = max<ll>(1, tr1 - 2); idx <= min<ll>(MAX, tr1 + 2);
               ++idx) {
            ld nscore = s(idx);
    ...
    opt();
    vector<pair<ld, int>> nscore2(m);
    for (int i = 0; i < m; ++i) {
      __int128 p = (__int128)a[c[i].u] * a[c[i].v];
      ld diff = p > c[i].val ? p - c[i].val : c[i].val - p;
      ld err = c[i].w * diff / c[i].val;
      nscore2[i] = {err, i};
    }
    nth_element(nscore2.begin(), nscore2.begin() + d, nscore2.end(),
                [](const pair<ld, int>& x, const pair<ld, int>& y) {
                  return x.x > y.x;
                });
    for (int i = 0; i < d; ++i) on[nscore2[i].y] = 0;
    opt();
    vector<pair<ld, int>> nscore(m);
    for (int i = 0; i < m; ++i) {
      __int128 p = (__int128)a[c[i].u] * a[c[i].v];
      ld diff = p > c[i].val ? p - c[i].val : c[i].val - p;
      ld err = c[i].w * diff / c[i].val;
    ...
    nth_element(nscore.begin(), nscore.begin() + d, nscore.end(),
                [](const pair<ld, int>& x, const pair<ld, int>& y) {
                  return x.x > y.x;
                });
    ld ntot = 0;
