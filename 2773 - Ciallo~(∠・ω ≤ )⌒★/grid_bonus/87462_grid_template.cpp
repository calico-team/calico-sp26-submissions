...
vector<int> solve(int N, vector<int>& A) {
    ...
    if(lis_up.size() >= lds_down.size()) {
        ...
        for (int i = 0; i < seq_up.size(); ++i) {
            ...
            for(int j=seq_up[i]+1; j<(i+1<seq_up.size() ? seq_up[i+1] : N-1); j++) {
                B[j] = A[seq_up[i]];
            }
        }
        for(int j=seq_up.back()+1; j<N; j++) {
            B[j] = A[seq_up.back()];
        }
    } else {
        ...
        for (int i = 0; i < seq_down.size(); ++i) {
            ...
            for(int j=seq_down[i]+1; j<(i+1<seq_down.size() ? seq_down[i+1] : N-1); j++) {
                B[j] = A[seq_down[i]];
            }
        }
        for(int j=seq_down.back()+1; j<N; j++) {
            B[j] = A[seq_down.back()];
        }
    }
    return B;
