...
signed main() {
    ...
    constexpr int min_iter = 6000, max_iter = 6000;
    ...
    while (T--) {
        ...
        for (int x = Xm - min_iter; x <= Xm; ++x) {
            for (int y = Ym - min_iter; y <= Ym; ++y) {
        ...
        for (int x = min_x; x <= Xm; ++x) {
            for (int y = Ym + 1; y <= Ym + max_iter; ++y) {
                if (2 * (abs(x - Xg) + abs(y - Yg)) > 7 * (abs(x - Xm) + abs(y - Ym)))
                    continue;
        ...
        for (int x = Xm + 1; x <= Xm + max_iter; ++x) {
            for (int y = min_y; y <= Ym; ++y) {
                if (2 * (abs(x - Xg) + abs(y - Yg)) > 7 * (abs(x - Xm) + abs(y - Ym)))
                    continue;
                else if (x >= min_x && x <= Xm && y >= Ym) {
                    const bool cond2 = 2 * (dist_min_x + dist(min_x, Ym, x, y)) > 7 * dist(Xm, Ym, x, y);
